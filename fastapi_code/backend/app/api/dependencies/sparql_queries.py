...
DELETE_SPECIES_LOCUS = (
    "DELETE WHERE {{ GRAPH <{0}> " "{{ <{1}> typon:isOfTaxon ?isOfTaxon .}} }}"
)

MULTIPLE_INSERT_SPECIES_LOCUS = (
    "INSERT {{ GRAPH <{0}> "
    "{{ ?locus a typon:Locus;"
...
ASK_SCHEMA_DEPRECATED = (
    'ASK WHERE {{ <{0}> typon:deprecated "true"^^xsd:boolean .}}'
)

ASK_SCHEMA_DEPRECATED2 = (
    "ASK WHERE {{ <{0}> a typon:Schema;"
...
ASK_SCHEMA_DATE = (
    "ASK WHERE {{ <{0}> a typon:Schema;" ' typon:{1} "{2}"^^xsd:dateTime .}}'
)
...
DELETE_USER = (
    "DELETE WHERE {{ GRAPH <{0}> {{ ?user a <http://xmlns.com/foaf/0.1/Agent>; "
    "typon:Role ?role . FILTER ( ?user = <{1}> ) }} }}"
)
...
COUNT_SINGLE_SCHEMA_LOCI_ALLELE = (
    "SELECT "
...
COUNT_SINGLE_SCHEMA_LOCI_ALLELES = (
    "SELECT "
    "(COUNT(DISTINCT ?locus) AS ?nr_loci) "
...
SELECT_SCHEMA_LOCI_ANNOTATIONS = (
    "SELECT DISTINCT "
    "?locus "
    "?name "
    "?UniprotName "
...
INSERT_SCHEMA_DATE = (
    "INSERT DATA IN GRAPH <{0}> " '{{ <{1}> typon:{2} "{3}"^^xsd:dateTime .}}'
)
